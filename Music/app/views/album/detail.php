...
?>
	...
	<div class="sub-container">
		...
		<img class="w3-spin w3-circle" style=" animation-duration: 4.5s;" src="<?php echo $value["Anh"] ?>" alt="music-playlist">
				...
				<center><h2><?php echo $value["tenList"]?></h2></center>
...
<b><span style="color:#090F9B; font-size: x-large;;"><center>Danh sách nhạc</center></span></b></h2>
	...
	?>
        <li class="current-song"><a href="<?php echo $listsong["link"]?>"><button>
            <span><img src="<?php echo $listsong["Anh"]?>" style="width:40px; height:40px; margin-left:5px;"></span>
            <?php echo $listsong["tenBH"]?>
			 <span style="position: absolute; right:35%;">
				 <?php
			$nghesi = $this->Model->fetchOne("select * from nghesi where maCS=".$listsong["maCS"]);
				if(isset($nghesi["maCS"]))
					echo $nghesi["tenCS"];
				 ?>
			</span>
            <span style="position: absolute; right:15%;"><?php echo substr($listsong["dateTime"],0,10)?></span></button></a></li><?php }}?>
    </ol>

		<hr>
<h2>Playlist cùng chủ đề</h2>
	<div class="swiper-container">
    <div class="swiper-wrapper">
	 <?php
		  foreach($data as $value ){
	  $data8 = $this->Model->fetch("select * from listbh where maAB =".$value["maAB"]." and maList !=".$value["maList"]." limit 10");
		foreach($data8 as $key){
	?>
       <div class="swiper-slide">
            <div class="single-square">
			  <div class="profile-pic">
				  <a href="index.php?controller=song/detail&id=<?php echo $key["maList"] ?>">
