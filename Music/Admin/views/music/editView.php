		...
		<div class="panel-body">
			<form action="index.php?controller=music/edit&act=edit&id=<?php echo $value["maBH"] ?>" method="post" enctype="multipart/form-data">
				...
				<div class="col-md-9">
					...
					<div style="margin-bottom: 10px;">
						<select name="list" class="form-control" style="margin-top: 10px; padding: 3px;">
								<?php
							$data = $this->Model->fetch("select * from listbh");
							foreach ($data as $key) {
						?>
							<option value="<?php echo $key["maList"]?>"
								<?php echo isset($value["maList"])&&$value["maList"]==$key["maList"]?"selected":""; ?>
							><?php echo $key["tenList"] ?></option>
						<?php } ?>
							</select>
							</select>
					</div>
