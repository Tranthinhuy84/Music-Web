		...
		<div class="panel-body">
			<form action="index.php?controller=music/add&act=add" method="post" enctype="multipart/form-data">
				<table cellpadding="3" style="width: 100%;">
					...
					<tr>
						<th>Danh sách playlist </th>
						<td>
							<select name="list" class="form-control" style="margin-top: 10px;margin-bottom:10px; padding: 3px;">
								<?php
									$data = $this->Model->fetch("select * from listbh");
									foreach ($data as $value) {
								?>
								<option value="<?php echo $value["maList"];?>"><?php echo $value["tenList"];?></option>
								<?php } ?>
							</select>
						</td>
					</tr>
