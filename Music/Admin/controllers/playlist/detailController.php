		...
		public function __construct(){
			...
			switch ($act) {
				case 'delete':
					...
					$this->Model->execute("update song set maList=0 where maBH='$baihat'" );
					echo "<meta http-equiv='refresh' content='0; URL=index.php?controller=playlist/detail&id=$malist'>";
					break;
			}
			$id = isset($_GET["id"])?$_GET["id"]:0;
			// dem tong so ban ghi
			$number = $this->Model->count("select * from song where maList='$id'");
			...
			$data = $this->Model->fetch("select * from song where maList='$id' order by maBH asc limit $form,$num_page");
