		...
		public function __construct(){
			...
			switch ($act) {
				case 'add':
					...
					if($_FILES["song"]["name"]){
						$link = "public/music/".$_FILES["song"]["name"];
					}
					if($moi=="Có")
					{
						$moi=1;
					...
					$this->Model->execute("insert into song(Anh, tenBH, maCD, maCS, maList, moi, link) values('$image','$name', '$catalog',  '$description', '$list', '$moi', '$link')");
